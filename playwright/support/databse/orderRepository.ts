...
export async function deleteOrderCheckout(orderEmail: string, orderDocument: string) {
  const customerCpf = orderDocument.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')

  await db
    .deleteFrom('orders')
    .where('customer_email', '=', orderEmail)
